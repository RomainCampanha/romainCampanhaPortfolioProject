...
export default function Home() {
  const [currentPage, setCurrentPage] = useState<"home" | "hobby">("home");
  const [scrollPositions, setScrollPositions] = useState({ home: 0, hobby: 0 });
  const isRestoringScroll = useRef(false);

  // Sauvegarder la position du scroll avant de changer de page
  const handlePageChange = (newPage: "home" | "hobby") => {
  ...
  return (
    <>
      ...
      <div className="transition-opacity duration-300">
        {currentPage === "home" ? <HomePage /> : <HobbyPage />}
      </div>
    </>
  );
}