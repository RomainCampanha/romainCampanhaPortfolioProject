...
export default function HomePage() {
  ...
  // Phase 5 : Contact — icônes + bulle loop (70% → 92%) — tout sur une seule page
  const showContact = progress >= 0.70 && progress < 0.92;
  const contactProgress = Math.max(0, Math.min(1, (progress - 0.70) / 0.08));
  ...
  return (
    <main className="min-h-dvh bg-gradient-to-b from-[#0A0A1F] via-[#2A153A] to-[#00C1FF]">
      ...
      {/* 📩 Section Contact : Icônes + Bulle loop — tout sur une seule page */}
      {showContact && (
        <div className="fixed inset-0 z-30 pointer-events-none flex flex-col items-center"
             style={{ paddingTop: isMobile ? '15vh' : '12vh' }}>

          {/* Icônes LinkedIn + Instagram */}
          <div className="flex gap-8 md:gap-12"
               style={{
                 opacity: contactProgress,
                 transform: `translateY(${(1 - contactProgress) * 40}px)`,
               }}>
            ...
            >
              <div className="flex flex-col items-center gap-2 group cursor-pointer">
                <div className="w-20 h-20 md:w-28 md:h-28 rounded-2xl
                ...
                <span className="text-white/80 font-orbitron text-[10px] md:text-sm tracking-wider
          ...
          {/* Bulle "Contacte moi !" en loop — sous les icônes */}
          <div className="mt-10 md:mt-12 pointer-events-auto"
               style={{
                 opacity: contactProgress,
                 transform: `translateY(${(1 - contactProgress) * 30}px)`,
               }}>
            <ChatBubble
              text="Contacte moi ! 📩"
              loop={true}
